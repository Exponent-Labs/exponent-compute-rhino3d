...
def Compute(closedPlanarCurve, multiple=False):
    ...
    if multiple:
        args = [[item] for item in closedPlanarCurve]
...
def Compute1(closedPlanarCurve, planarTolerance, multiple=False):
    ...
    if multiple:
        args = zip(closedPlanarCurve, planarTolerance)
    response = Util.ComputeFetch(url, args)
    return response
...
def Compute4(mesh, area, firstMoments, secondMoments, productMoments, multiple=False):
    ...
    if multiple:
        args = list(zip(mesh, area, firstMoments, secondMoments, productMoments))
    response = Util.ComputeFetch(url, args)
    return response
...
def Compute6(brep, area, firstMoments, secondMoments, productMoments, multiple=False):
    ...
    if multiple:
        args = list(zip(brep, area, firstMoments, secondMoments, productMoments))
    response = Util.ComputeFetch(url, args)
    return response
...
def Compute7(surface, multiple=False):
    ...
    args = [surface]
    if multiple:
        args = [[item] for item in surface]
    response = Util.ComputeFetch(url, args)
    return response


def Compute8(
    surface, area, firstMoments, secondMoments, productMoments, multiple=False
):
    """
    ...
    if multiple:
        args = list(zip(surface, area, firstMoments, secondMoments, productMoments))
    response = Util.ComputeFetch(url, args)
    return response
...
def Compute9(geometry, multiple=False):
    ...
    args = [geometry]
    if multiple:
        args = [[item] for item in geometry]
    response = Util.ComputeFetch(url, args)
    return response


def Compute10(
    geometry, area, firstMoments, secondMoments, productMoments, multiple=False
):
    """
    ...
    if multiple:
        args = list(zip(geometry, area, firstMoments, secondMoments, productMoments))
    response = Util.ComputeFetch(url, args)
    return response
